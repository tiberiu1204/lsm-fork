    ...
    with open(LOG_FILENAME, "a", buffering=1) as log_file:
        ...
        try:
            for filename in pbar:
                ...
                # Construct command
                cmd = [FILTER_WX_BIN, JS_BIN, file_path]
                ...
                try:
                    ...
                    log_file.write(f"--- FINISHED: {filename} (Exit Code: {process.returncode}) ---\n\n")

                except subprocess.TimeoutExpired:
                    # Handle the timeout case
                    process.kill()
                    log_file.write(f"\n--- TIMED OUT ({timeout_seconds}s): {filename} ---\n\n")
                    # No need to print to console, tqdm handles the update, 
                    # but we can force a refresh if needed or just let it proceed.
...
if __name__ == "__main__":
    # Argument parsing
    parser = argparse.ArgumentParser(description="Run malware samples with a timeout.")
    parser.add_argument(
        "--timeout", 
        type=int, 
        default=600, 

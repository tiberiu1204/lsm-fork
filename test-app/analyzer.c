...
int analyze_nop(unsigned long addr, unsigned long len) { return 0; }
...
int analyze_yara(unsigned long addr, unsigned long len) {
  int result = 0;
  yr_rules_scan_mem(yr_rules, (const uint8_t *)addr, len,
                    0, // flags
                    yara_callback,
                    &result, // user_data
                    0        // timeout
...
static int send_int_msg(struct nl_sock *sk, int fam, int value) {
  ...
  void *hdr =
      genlmsg_put(msg, NL_AUTO_PORT, NL_AUTO_SEQ, fam, 0, 0, LSM_CMD_REPLY, 1);
  if (!hdr) {
    return -EMSGSIZE;
  }
...
int main(int argc, char **argv) {
  ...
  int mcgrp = genl_ctrl_resolve_grp(sock, GEN_NETLINK_FAMILY_NAME,
                                    GEN_NETLINK_GROUP_NAME);
  if (mcgrp < 0) {
  ...
  if (nl_socket_modify_cb(sock, NL_CB_VALID, NL_CB_CUSTOM, message_handler,
                          NULL) < 0) {
